...
export default function RequestWizard({ categories, addresses }: RequestWizardProps) {
  ...
  const [address, setAddress] = useState(addresses[0]?.line1 ?? "");
          ...
          <form action={formAction} className="mt-6 space-y-4">
            ...
            {step === "address" && (
              <div className="space-y-4">
                ...
                >
                  {addresses.map((saved) => (
                    <option key={saved.id} value={saved.line1}>
      ...
      <Card className="bg-slate-900 text-white">
        ...
        <CardContent className="space-y-4 text-sm text-slate-100">
          ...
          <div className="flex items-start gap-2">
            ...
            <p className="text-sm text-slate-200">{address || "Add a delivery address"}</p>
          </div>
        </CardContent>
      </Card>
    </div>
